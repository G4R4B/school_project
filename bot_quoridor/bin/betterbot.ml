...
let value_of_the_ennemy_move () =
  let have_list_of_player = try have_all_player_in_board () with Not_found -> raise (Failure "have_all_player") in
  let rec find_the_distance_player_most_on_final_line lst acc = try match lst with 
    | [] -> acc
    ...
    with Not_found -> raise (Failure "find_the_distance")
  in
  let int_distance = find_the_distance_player_most_on_final_line have_list_of_player max_int in
  let rec find_the_list_of_player_most_on_the_final_line lst acc color = try match lst with 
    | [] -> color
    | (x,y)::q -> let value = distance (color_to_final_pos ((match get_color_of_a_position (x,y) with | Some p -> p | None -> current_player()).color )) (x,y) in
    ...
    with Not_found -> raise (Failure "find_the_list")
  in
...
in
  ...
  match (List.find_opt (fun (x,y) -> (match get_color_of_a_position (x,y) with | Some p -> p | None -> current_player()).color = color) ) list
  with 
  | Some p -> p 
  | None -> List.hd list (*HERE THE PREVIOUS BUG WITH NOT_FOUND*)
in
...
let value_of_the_ennemy_move_with_the_wall (x1,y1) (x2,y2) player_choose =
  ...
  let find_the_pos_of_the_color list = 
    match (List.find_opt (fun (x,y) -> (match get_color_of_a_position (x,y) with | Some p -> p | None -> current_player()).color = player_choose) ) list
  with 
  | Some p -> p 
  | None -> List.hd list (*HERE THE PREVIOUS BUG WITH NOT_FOUND*)
in
        ...
        List.for_all
          (fun pos_vect ->
            (*Format.printf "pos: %d,%d"
              ...
              (pos_vect = pos_of_my_wall || pos_vect = pos_of_my_wall2) || (try is_wall (fst pos + fst pos_vect, snd pos + snd pos_vect)
...
let test_move (*nb_player*) pos =
  ...
  let lstMv = try list_of_moves pos with Not_found -> raise (Failure "Not_found list_of_moves") in
  try match List.find_opt  (fun (x,y) -> in_final_line (x,y) (color_to_final_pos (current_player ()).color)) lstMv with
  | Some (x,y) -> Moving (x,y)
  | None -> (
  let lstWall = try list_of_wall () with Not_found -> raise (Failure "Not_found list_of_wall") in
  
  let rec aux test_all_moves acc value_of_acc =
    try( match test_all_moves with
    | [] -> acc,value_of_acc
    ...
    ) with Not_found -> raise (Failure "Not_found test_all_moves")
  ...
  (*Format.printf "pos : (%d,%d)\n" x y; *)
  let value_of_the_ennemy_move,player_choose = try value_of_the_ennemy_move () with Not_found -> raise (Failure "Not_found value_of_the_ennemy_move") in
  (*Format.printf "value_of_the_ennemy_move : %d\n" value_of_the_ennemy_move;*)
  let rec aux2 test_all_wall acc value_of_acc = try match test_all_wall with
    | [] -> acc,value_of_acc
    ...
    with Not_found -> raise (Failure "Not_found test_all_wall")
  ...
  if ((distance (color_to_final_pos (current_player ()).color) pos )/2 -  interest_of_the_move) > interest_of_the_wall then Moving (x,y)
    ...
    if will_wall_block_player_for_bot (have_all_player_in_board ()) (x1,y1) (x2,y2) then Moving(x,y) else Placing_wall ((x1,y1),(x2,y2))
  )
  )
  with Not_found -> raise (Failure "Not_found find_opt")



(** This function defines our strategy for my player*)
let better_move pos =
  try test_move pos with Not_found -> raise (Failure "here")