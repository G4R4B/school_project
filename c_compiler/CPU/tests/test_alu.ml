...
let _test_alu =
  ...
  let test expected (a,b,c,x,y) =
    ...
    print_int c ;
    print_string " -> " ;
    print_endline (string_of_int_list (nb_vers_bits expected)) ;
    print_string "Resultat : " ;
    print_endline (circuit (Array.of_list (a::b::c::nb_vers_bits x@nb_vers_bits y)) |> Array.to_list |> string_of_int_list) ;
    print_newline () ; *)
    assert(
