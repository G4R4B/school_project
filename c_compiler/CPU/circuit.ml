...
let tri_topologique vient_apres =
  ...
  let rec libere noeud =
    if rang.(noeud) = -1
    then print_endline (string_of_int noeud);
...
let compile_raw entrees sorties =
  let nb_points, _zero, _un, portes_nand, portes_delai, entrees, sorties = compacte_cc entrees sorties in
  let types = type_points entrees nb_points portes_nand portes_delai in
  (types,portes_nand,portes_delai,entrees,sorties)
